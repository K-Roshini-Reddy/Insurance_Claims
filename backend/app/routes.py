...
def ready(request: Request):
    ...
    required = s.get("model", {}).get("required", False)
...
def model_info(request: Request):
    res = getattr(request.app.state, "model_res", None)
    if not res:
        return {"loaded": False, "error": "model not initialized"}
    ...
    return {
        ...
        "challenger": {
            "enabled": bool(ab.get("enabled", False)) and (ab.get("mode") or "shadow").lower() == "shadow",
